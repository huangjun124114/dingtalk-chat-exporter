...
### 新增
- **定时任务运行状态灯**：任务卡片头部（「时间间隔」与「开关」之间）新增状态指示灯
  - 🔴 红灯（呼吸闪烁）= 当前有任务在运行（本任务 / 其它定时任务 / 手动导出）
  - 🟢 绿灯 = 空闲
  - ⚫ 黑灯 = 上次运行失败（`error` / `partial`）
- **运行/停止按钮随状态切换**：全局任务槽被占用时，「立即运行」自动变为「停止运行」；点击后进入「停止中…」禁用态，任务真正结束后自动恢复为「立即运行」
- **`get_scheduler_status` 命令**：返回 `running` / `runningKind`（export|schedule）/ `runningScheduleId` / `stopping`，供定时页 2 秒轮询
- **运行记录新增「结束时间」列**：运行中的记录显示「进行中…」，未结束显示 `—`

### 改进
- **运行态实时化**：定时页进入时启动 2 秒轮询，离开时自动停止；运行结束的那一刻自动刷新卡片数据（运行记录 / 水位线 / 下次运行时间）
- **终止语义修正**：用户手动终止的运行记录状态由 `error` 改为 `cancelled`（界面显示「已终止」），不再与真实失败混淆；终止时按运行记录定位，其它任务在跑时也能正确终止
- **终止入口统一**：在主界面点击「停止运行」会终止当前真正在跑的任务（手动导出走 `cancel_export`，定时任务走 `cancel_schedule_run`）
